...
import { REQUEST_PAYMENT_METHODS } from "../../constants";
...
export const fetchData = async (options) => {
  try {
    const [ciudadesResponse, tiemposCompraResponse, metodosPagoResponse] =
      await Promise.all([
    ...
    const tiemposCompra = tiemposCompraResponse.data.data;
    ...
    return { ciudades, tiemposCompra, metodosPago };
  } catch (error) {
